...
export default function EditListing() {
  ...
  useEffect(() => {
    const loadProduct = async () => {
      try {
        ...
        if ((product as any).user_id !== user.id) {
        ...
        setFormData({
          ...
          type: productData.type as 'Venda' | 'Aluguel',
          period: (productData.period as 'Diário' | 'Semanal' | 'Mensal' | null) || undefined,
          brand: productData.brand,
          model: productData.model,
          year: productData.year,
          category: productData.category,
          location: productData.location,
          hours: productData.hours?.toString() || '',
          power: productData.power?.toString() || '',
          implementType: productData.implement_type || '',
          workWidth: productData.work_width?.toString() || '',
          partType: productData.part_type || '',
          partCondition: productData.part_condition || '',
        ...
        setError('Erro ao carregar anúncio. Por favor, tente novamente.');
        setLoadingProduct(false);
      }
    };
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    files.forEach(file => {
      ...
      setImages(prev => [...prev, { file, preview, uploading: false, isExisting: false }]);
    });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log('🚀 Iniciando atualização de anúncio...');
      
      if (images.length === 0) {
        throw new Error('Por favor, mantenha pelo menos uma foto do produto.');
      }
      ...
      for (let i = 0; i < images.length; i++) {
        ...
        if (image.isExisting && image.url) {
          ...
          console.log(`📤 Uploading nova imagem ${i + 1}/${images.length}...`);
      ...
      // Deletar todas as imagens adicionais antigas
      ...
      if (deleteImagesError) {
        console.error('⚠️ Erro ao deletar imagens antigas:', deleteImagesError);
      }

      // Adicionar novas imagens adicionais
      ...
      console.log('🎉 Anúncio atualizado com sucesso! Redirecionando...');
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 mt-4">Carregando anúncio...</p>
        </div>
      </div>
    );
  }
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="bg-white rounded-lg shadow-md overflow-hidden">
              <div className="p-6 border-b border-gray-100">
                ...
                <p className="mt-1 text-sm text-gray-500">
                  Atualize os detalhes específicos do seu equipamento
                </p>
              </div>
