...
-- Create simplified name validation function
...
BEGIN
  ...
  FOR i IN 1..array_length(name_parts, 1) LOOP
    -- Skip checking length for prepositions
    IF name_parts[i] = ANY(ARRAY['de', 'da', 'do', 'dos', 'das', 'e']) THEN
      CONTINUE;
    END IF;
    
    -- Each non-preposition part must be at least 2 characters
