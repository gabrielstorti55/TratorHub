...
BEGIN
  ...
  FOR i IN 1..array_length(name_parts, 1) LOOP
    ...
    IF name_parts[i] = ANY(ARRAY['de', 'da', 'do', 'dos', 'das', 'e', 'von', 'van', 'del', 'la', 'el']) THEN
      CONTINUE;
    END IF;
    ...
    -- Check if part contains at least one letter
    IF NOT (name_parts[i] ~ '[[:alpha:]]') THEN
      RETURN false;
    END IF;
  END LOOP;
  ...
  -- - Apostrophes (for names like D'Ávila)
  -- - Periods (for abbreviations like M.)
  RETURN cleaned_name ~ '^[[:alpha:]\s\u00C0-\u00FF''\-\.]+$'
    AND NOT cleaned_name ~ '[-''\.]{2,}' -- Prevent consecutive special characters
    AND NOT cleaned_name ~ '^[-''\.]|[-''\.]$'; -- Prevent special characters at start/end
END;
$$ LANGUAGE plpgsql;
