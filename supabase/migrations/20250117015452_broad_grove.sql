...
BEGIN
  ...
  FOR i IN 1..array_length(name_parts, 1) LOOP
    ...
    -- 2. Start with a letter (including accented)
    -- 3. End with a letter (including accented)
    -- 4. Allow letters, hyphens, and apostrophes in the middle
    IF length(name_parts[i]) < 2 
      OR name_parts[i] !~ '^[A-Za-zÀ-ÿ]'
      OR name_parts[i] !~ '[A-Za-zÀ-ÿ]$'
      OR name_parts[i] !~ '^[A-Za-zÀ-ÿ''\-]+$' THEN
      RETURN false;
    END IF;
  END LOOP;
  
  -- Final validation:
  -- 1. Must contain only letters, spaces, hyphens, and apostrophes
  -- 2. No consecutive special characters
  -- 3. Minimum total length of 5 characters
  RETURN cleaned_name ~ '^[A-Za-zÀ-ÿ\s''\-]+$'
    AND NOT cleaned_name ~ '[-'']{2,}'
    AND length(cleaned_name) >= 5;
END;
$$ LANGUAGE plpgsql;
